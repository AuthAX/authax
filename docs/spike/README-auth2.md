...
Consumer side OIDC is a future authentication strategy beside OTP and passkeys, already proven as an arbitrary third-party namespace in [`src/contracts-typecheck.ts`](./src/contracts-typecheck.ts). Making AuthAX an OIDC or OAuth provider is a separate identity server product and out of scope.
